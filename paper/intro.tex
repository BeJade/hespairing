...
\textbf{Chloe: I think this paragraph is a bit random and can be removed as the title of the next section already tells the reader what's coming}
...
it may be necessary to increase the size of the finite fields for pairing-friendly curves. Every optimised implementation of pairing-based algorithms makes use of the fact that some of the arithmetic can be performed over $\FF_{p}$ instead of the whole field $\FF_{p^{k}}$, so increasing the prime would significantly slow down the algorithm. We therefore prefer to increase the embedding degree $k$. 

%With current methods for pairing computations this would mean moving from $k=12$ to $k=18$ for 128-bit security, and from $k=18$ to $k=24$ for 196-bit security.
%
%However, the computations that cannot be performed over the base field $\FF_{p}$ will be much slower in this case, and $k$ is increased more than necessary for the required security level. We therefore present an algorithm to compute pairings in fields of embedding degree $k=15$ (for 128-bit security) and $k=21$ (for 196-bit security).
...
Every optimised implementation of pairing-based algorithms makes use of the fact that some of the arithmetic can be performed over $\FF_{p}$ instead of the whole field $\FF_{p^{k}}$, so increasing the size of the prime field has a penalty of slowing down arithmetic operations, which has a huge impact on performance.
