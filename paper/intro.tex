...
if $E$ is isomorphic over $\FF_{p}$ to a curve in Hessian or Edwards form,
then it is not isomorphic over $\FF_{p^{k}}$ to a curve in Hessian or Edwards form,
...
%have been shown previously to be competitive,
%for example in
...
This means increasing the conventional even embedding degree $k=12$ to $k=18$ or to $k=24$ for a higher security level.
%current standard (even) embedding degrees $k=12$ and $k=18$, on which we give more details below.
...
We present the first construction (to our knowledge) of a pairing on curves in twisted Hessian form with odd embedding degree.
...
that can be written in twisted Hessian form.
% and show how to use the degree 3 twists to optimize computations.
